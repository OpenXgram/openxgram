...
마스터 지시를 받은 에이전트는 아래 9단계(0~8)를 **반드시 순서대로** 수행한다.
list_peers(= `list_peer`)가 활성 세션의 단일 진리원천 — alias·session id·역할을 항상 실시간 조회한다.

0. 마스터의 지시가 들어온 경로를 확인하고, 마스터의 지시를 명확하게 파악한다.
1. 해당 지시가 자신의 역할에 맞지 않는 경우 이행할 에이전트를 확인한다(list-peers). 사용자가 대화명으로 지정하면 그 에이전트의 alias·세션id·역할을 확인한다. 잠자고 있으면 spawn 한다.
2. list-peers로 실제 활성 세션의 alias·세션id·역할을 확인한다.
3. 지시를 이행할 역할을 확인하고 해당 역할의 alias를 확인해 송신한다.
4. 해당 alias가 메시지를 수신하고 작업을 시작했는지 확인한다(송신=수신 아님, 전송 확인 필수).
5. 해당 alias가 지시를 이행한 결과를 수신한다.
6. 지시가 정확히 이행됐는지 확인하고, 부족·오류가 있으면 체크해서 해당 alias에게 다시 지시한다.
7. 마스터의 지시를 제대로 파악·수행할 때까지 순환한다.
8. 최종 결과가 도착하면 마스터 지시가 유입된 경로(API/A2A/webhook 내·외부, 음성 등)를 통해 역방향으로 결과를 보고한다.
