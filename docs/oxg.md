...
마스터 지시를 받은 에이전트는 아래 단계(0~8)를 **반드시 순서대로** 수행한다.
list_peers(= `list_peer`)가 활성 세션의 단일 진리원천 — alias·session id·역할을 항상 실시간 조회한다.

0. 마스터의 지시가 들어온 경로를 확인하고, 마스터의 지시를 명확하게 파악한다.
1a. 해당 지시가 자신의 역할에 맞지 않는 경우 해당 지시를 이행할 에이전트를 확인한다(list-peers). 사용자가 대화명으로 에이전트를 지정할 경우 해당 에이전트의 alias, 세션id, 역할을 확인한다. 해당 에이전트가 잠자고 있는 경우 spawn 한다.
1b. list-peers를 통해 실제 활성화된 세션의 alias와 세션 id, 역할을 확인한다.
2. 마스터의 지시를 이행하기 위한 에이전트의 역할을 확인하고 해당 역할을 하는 alias를 확인한다.
3. 해당 alias에게 마스터의 지시를 송신한다. (보낼 때 분명하고 정확하게 회신할 수 있도록 필요한 정보를 포함해서 보낼 것. 읽음 확인도 보내야 함)
4. 해당 alias가 메세지를 수신하고 작업을 시작했는지 확인한다. (읽음 확인이 회신되었는지 확인, 송신했다고 무조건 내용을 받은 것이 아니라는 것을 알고 제대로 전송되었는지 확인해야 한다.)
5. 해당 alias가 지시를 이행한 결과를 수신받는다.
6. 마스터의 지시가 정확하게 이행되었는지 확인하고, 부족하거나 잘못된 부분이 있으면 잘못된 부분을 체크해서 해당 alias에게 다시 지시한다.
7. 마스터의 지시를 제대로 파악하고 수행할 때까지 순환한다.
8. 최종 제대로 된 지시 결과가 도착하면 마스터의 지시가 유입된 경로를 통해 결과를 보고한다. [API(외부,내부)로 들어온 경우, A2A(외부,내부)로 들어온 경우, webhook(외부,내부)을 통해 들어온 경우, 음성 지시로 유입된 경우 등 유입된 경로를 통해서 반대로 결과를 보내야 한다.]

### 참고
- P2P 통신이 불능이거나, 통신 상태가 원활하지 않은 경우 3회 재시도 후에 webhook으로 전환하여 직접 주입한다. (passcode: zalman 0205, seoul 9386)
- 현재 portal의 webhook은 헤더 보안으로 전환 중임.

### list-peer 로스터 규칙
- list-peer에는 에이전트의 **대화명, alias, 세션 id, 역할, 폴더 위치, 세션 활성화 상태**가 표시되어야 한다.
