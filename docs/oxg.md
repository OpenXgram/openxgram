...
마스터 지시를 받은 에이전트는 아래 단계(1~11)를 **반드시 순서대로** 수행한다.
list_peers(= `list_peer`)가 활성 세션의 단일 진리원천 — 대화명(title)·alias·session id·역할·폴더 위치·활성화 상태를 항상 실시간 조회한다.

1. 마스터의 지시가 들어온 경로를 확인하고, 마스터의 지시를 명확하게 파악한다.
2. 해당 지시가 자신의 역할에 맞지 않는 경우 해당 지시를 이행할 에이전트를 확인한다(list-peers). 사용자가 대화명으로 에이전트를 지정할 경우 해당 에이전트의 alias, 세션id, 역할을 확인한다. 해당 에이전트가 잠자고 있는 경우 spawn 한다.
3. list-peers를 통해 실제 활성화된 세션의 alias와 세션 id, 역할을 확인한다.
4. 마스터의 지시를 이행하기 위한 에이전트의 역할을 확인하고 해당 역할을 하는 alias를 확인한다.
5. 해당 alias에게 마스터의 지시를 송신한다. (보낼 때 분명하고 정확하게 회신할 수 있도록 필요한 정보를 포함해서 보낼 것. 읽음 확인도 보내야 함)
6. 읽음 확인 회신이 오지 않은 경우 1분간 대기하고 재발신 한다.
7. 해당 alias가 메세지를 수신하고 작업을 시작했는지 확인한다. (읽음 확인이 회신되었는지 확인, 송신했다고 무조건 내용을 받은 것이 아니라는 것을 알고 제대로 전송되었는지 확인해야 한다.)
8. 해당 alias가 지시를 이행한 결과를 수신받는다.
9. 마스터의 지시가 정확하게 이행되었는지 확인하고, 부족하거나 잘못된 부분이 있으면 잘못된 부분을 체크해서 해당 alias에게 다시 지시한다.
10. 마스터의 지시를 제대로 파악하고 수행할 때까지 순환한다.
11. 최종 제대로 된 지시 결과가 도착하면 마스터의 지시가 유입된 경로를 통해 결과를 보고한다. [API(외부,내부)로 들어온 경우, A2A(외부,내부)로 들어온 경우, webhook(외부,내부)을 통해 들어온 경우, 음성 지시로 유입된 경우 등 유입된 경로를 통해서 반대로 결과를 보내야 한다.]

### 참고
- P2P 통신이 불능이거나, 통신 상태가 원활하지 않은 경우 3회 재시도 후에 webhook으로 전환하여 직접 주입한다. (passcode: 각 머신의 `daemon.env` webhook passcode 참조 — 비공개, 레포 비포함)

### list-peer 로스터 규칙
- list-peer에는 에이전트의 **대화명(title), alias, 세션 id, 역할, 폴더 위치, 세션 활성화 상태**가 표시되어야 한다.
- 모든 에이전트는 세션이 시작될 때 자신의 대화명, alias, 세션 id, 역할, 폴더 위치, 세션 활성화 상태를 list-peer에 업데이트해야 한다.
- 세션이 시작하고 list-peer를 업데이트할 때, 자신의 대화명 또는 alias가 일치하는 기존 정보가 있으면 삭제하고 새롭게 갱신해야 한다.

### 통신 시 자기 등록 확인 (강제)
- 사용자가 에이전트 간 통신을 말하면 **가장 먼저** 자신의 정보가 list_peer에 제대로 등록되어 있는지 확인한다.
- 등록 상태가 정확하더라도 통신 직전에 한 번 더 업데이트하여 stale 정보 방지.
- 등록 누락/오래된 경우, 통신 시도 전에 즉시 `register_subagent`(또는 동등 도구)로 갱신.

### 읽음 확인 + 재발송 (강제)
- 모든 메세지는 발송 후 **읽음 확인 회신을 받아야 한다.**
- 1분 내에 읽음 확인이 회신되지 않은 경우, 재발송하고 읽음 확인 회신을 재요청한다.
- 3회 재발송에도 읽음 확인이 오지 않으면 P2P 불능으로 간주 — webhook fallback으로 전환.

### 작업 시한 규칙 (10분 룰)
- 모든 작업은 평균 10분을 넘지 않는다.
- 메세지 발송 후 10분을 초과할 경우, 완료 회신이 올 때까지 **10분 단위로 진행 상태 보고를 받는다.**
- 보고가 오지 않으면 다시 progress 요청을 보낸다.
