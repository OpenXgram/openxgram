...
- 신규: 24단어 BIP39 니모닉 표시 후 마스터가 다른 곳에 기록했는지 확인 (Y/N)
- import: 24단어 직접 입력
...
- keystore 저장 위치: `~/.starian/keystore/master.json` (600 권한)
...
- BGE-small-en-v1.5 모델 다운로드 (캐시: `~/.starian/models/`)
- 모델 SHA256 해시 검증
...
- 백업 파일 생성: `~/.starian-backup-YYYYMMDD-HHMMSS.tar.gz.enc` (KST 기준)
- AES-256-GCM으로 암호화. 키: 마스터 시드에서 파생
- 백업 파일 SHA256 출력 (마스터가 기록 가능)
- 백업 파일 크기 및 위치 출력 후 마스터 확인

옵션 3 — 부분 보존 (특정 경로 명시)
- 마스터가 보존할 경로 목록 입력 (예: `~/.starian/sessions/`, `~/.starian/keystore/`)
...
- `~/.starian/` 전체를 trash로 이동 (`rm -rf` 절대 금지)
- Linux: XDG trash (`~/.local/share/Trash/`), trash-cli 사용
- macOS: `~/.Trash/` native trash API
...
검사 범위: 홈 디렉토리 (기본값). `--scan-system` 플래그로 시스템 전체 확장 가능.
...
단계 2 — DB 백업
- 현재 DB를 `~/.starian/backup/pre-v{새버전}-{KST타임스탬프}.sqlite`로 복사
...
- 기존 바이너리를 `~/.starian/backup/xgram-v{이전버전}`으로 백업
- 새 바이너리로 교체
...
- 데몬 정지
- `~/.starian/backup/pre-v{VERSION}-*.sqlite` 최신 파일을 현재 DB로 복원
- install-manifest.json의 version 필드를 이전 버전으로 복원
- 이전 바이너리를 `~/.starian/backup/xgram-v{VERSION}`에서 복원
- 데몬 재시작 + 헬스체크
...
- `--keep-data PATH`: 지정 경로 보존 (반복 사용 가능). 예: `--keep-data ~/.starian/sessions/`
- `--dry-run`: 실제 변경 없이 수행될 작업 목록만 출력
- `--scan-system`: 흔적 검사를 홈 디렉토리에서 시스템 전체로 확장
...
- 반드시 `# BEGIN OPENXGRAM` / `# END OPENXGRAM` 마커 쌍으로 블록을 감싼다.
...
- 실제 파일 시스템 변경 없음 확인 (`ls ~/.starian` 실패 또는 존재하지 않음)
...
- `~/.starian-backup-*.tar.gz.enc` 생성 확인
- 백업 파일을 다른 머신으로 복사
- 다른 머신에서 `xgram init --import-backup ~/.starian-backup-*.tar.gz.enc` 실행 (Phase 1.5)
- 복원된 세션 데이터 접근 가능 확인
...
- keystore 모듈: 시드 로드, EdDSA 서명/검증, keystore 패스워드 암호화. uninstall_token 생성/검증에 필수.
- DB 마이그레이션: sqlx-cli 또는 자체 마이그레이션 러너. §8의 순차 SQL 적용에 사용.
- 외부 어댑터 모듈: Discord/Telegram 토큰 검증 API 호출. Step 7 및 doctor 항목 7~8에 사용.

### 15.2 외부 크레이트

- `keyring`: OS 키체인 추상화 (macOS Keychain, Windows Credential Manager, Linux Secret Service)
- `trash`: trash 이동 추상화 (Linux trash-cli, macOS native, Windows Recycle Bin)
...
마스터에게 확인이 필요한 미결 사항이다.

결정 1 — 데이터 디렉토리 기본 경로
- 후보 A: `~/.starian/` (기존 Starian 생태계와 일치)
- 후보 B: `~/.openxgram/` (프로젝트 이름 명시)
- 후보 C: `$XDG_DATA_HOME/openxgram/` (Linux XDG 표준 준수)
- 영향: manifest, 문서, 백업 파일명 모두 이 경로를 기준으로 함

결정 2 — OS 서비스 등록 기본값
- 후보 A: 자동 등록 (마스터 확인 없이 Step 9에서 바로 등록)
- 후보 B: 마스터 확인 후 등록 (기본 Y, 엔터로 넘어가면 자동)
- 후보 C: 등록 안 함 (마스터가 수동으로 `xgram enable-service` 실행)
- 영향: 재부팅 후 자동 시작 여부, 보안 모델

결정 3 — BIP39 시드 백업 강제 여부
- 후보 A: 강력 강제 (24단어를 화면에 표시하고, 마스터가 세 단어를 임의로 재입력하여 검증)
- 후보 B: 약한 강제 (Y/N 확인만, 실제 검증 없음)
- 후보 C: 강제 없음 (경고만 출력하고 진행)
- 영향: 시드 분실 시 데이터 복구 불가 위험도

결정 4 — Linux trash 라이브러리
- 후보 A: `trash-cli` 시스템 패키지 의존 (설치 안 되어 있으면 apt/dnf 안내)
- 후보 B: XDG trash 스펙 직접 구현 (외부 의존성 없음)
- 후보 C: `trash` Rust 크레이트가 XDG 구현 포함 여부 확인 후 결정
- 영향: 설치 복잡도, 패키지 의존성

결정 5 — 흔적 검사 범위
- 후보 A: 홈 디렉토리만 (`$HOME`) — 기본값
- 후보 B: `--scan-system` 플래그로 시스템 전체 선택 가능
- 후보 C: 항상 시스템 전체 (느리지만 확실)
- 영향: uninstall 시간, system-level 설치 시 잔존 흔적 감지 여부

결정 6 — backup 보관 위치 기본값
- 후보 A: `~/` (홈 루트) — `~/.starian-backup-*.tar.gz.enc`
- 후보 B: `~/.starian/backup/` (데이터 디렉토리 내)
- 후보 C: `$XDG_DATA_HOME/openxgram/backup/`
- 영향: 데이터 디렉토리 제거 시 백업도 함께 trash되는 문제 (후보 B의 경우)

결정 7 — 다른 머신 sync 시 인증 방식
- 후보 A: Tailscale 인증서 기반 mTLS (Tailscale 사용 시만 가능)
- 후보 B: 마스터 시드 챌린지-응답 (두 머신이 같은 시드 보유 시 상호 인증)
- 후보 C: 단방향 토큰 (송신 머신이 일회성 토큰 발급, 수신 머신이 검증)
- 영향: 보안 강도, 다른 시드를 가진 머신으로도 sync 가능 여부
