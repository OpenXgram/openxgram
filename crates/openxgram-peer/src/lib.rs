...
impl<'a> PeerStore<'a> {
    ...
    /// 새 peer 등록. alias / public_key 가 unique.
    pub fn add(
        &mut self,
        alias: &str,
        public_key_hex: &str,
        ...
        self.db.conn().execute(
            "INSERT INTO peers (id, alias, public_key_hex, address, role, created_at, notes)
    ...
    pub fn get_by_alias(&mut self, alias: &str) -> Result<Option<Peer>> {
        Self::row_to_opt(self.db.conn().query_row(
            "SELECT id, alias, public_key_hex, address, role, last_seen, created_at, notes
...
fn tuple_to_peer(
    ...
    let (id, alias, public_key_hex, address, role, last_seen, created_at, notes) = t;
    Ok(Peer {
        id,
        alias,
        public_key_hex,
